  ...
  return (
    <Popover open={opened} onOpenChange={setOpened}>
      <PopoverTrigger asChild>
        <Button variant="ghost" size="sm" className="h-7 px-2 text-xs text-purple-300 hover:bg-purple-500/10 hover:text-purple-200" onClick={(e) => e.stopPropagation()} title="Ver provenance per-property">
      ...
      <PopoverContent className="w-96 max-h-96 overflow-y-auto bg-card border-foreground/10 text-gray-100" onClick={(e) => e.stopPropagation()}>
        ...
        {isLoading && <div className="flex justify-center py-4"><Loader2 className="h-5 w-5 animate-spin text-gray-500" /></div>}
        {error && <div className="text-xs text-red-400 py-2 flex items-start gap-1"><AlertTriangle className="h-3 w-3 mt-0.5 shrink-0" />{error instanceof Error ? error.message : 'Error'}</div>}
                ...
                <div key={prop.prop} className={`px-2 py-1 rounded border ${prop.conflict ? 'bg-red-500/10 border-red-500/30' : 'bg-foreground/5 border-foreground/10'}`}>
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-gray-500">{prop.statements.length} <Database className="inline h-2.5 w-2.5" /></span>
                    </div>
                  </div>
                  ...
                  <div className="text-[10px] text-gray-500 mt-0.5">
                    ...
                    {prop.policy && <span className="ml-1 text-purple-400">[{prop.policy}]</span>}
                  </div>
                </div>
              ))}
              {Object.keys(data.properties).length > 6 && <div className="text-center text-gray-500 text-[10px] pt-1">+{Object.keys(data.properties).length - 6} mas — abre "Completo" para ver todas</div>}
            </div>
          </div>
        )}
      </PopoverContent>
