...
export function EntitySourceRecordsCard({
  ...
  const catColors: Record<string, string> = {
    ...
    LAW_ENFORCEMENT: 'bg-orange-500/10 text-orange-700 dark:text-orange-400 border-orange-500/20',
    PEP: 'bg-purple-500/10 text-purple-600 dark:text-purple-400 border-purple-500/20',
    REGULATORY: 'bg-amber-500/10 text-amber-700 dark:text-amber-400 border-amber-500/20',
    TAX: 'bg-yellow-500/10 text-yellow-700 dark:text-yellow-400 border-yellow-500/20',
    DEBARMENT: 'bg-pink-500/10 text-pink-600 dark:text-pink-400 border-pink-500/20',
  };
        ...
        <div className="space-y-2">
          {sourceRecords.map((rec, i) => (
            <div key={i} className="flex items-center gap-3 p-2.5 rounded-lg bg-foreground/[0.03]">
              {rec.category && (
                <Badge variant="outline" className={cn('text-[10px] shrink-0', catColors[rec.category] || 'bg-foreground/5 text-muted-foreground')}>
              ...
              <div className="flex-1 min-w-0">
                ...
                {rec.country && (
                  <span className="text-[10px] text-muted-foreground ml-2">{countryNames[rec.country] || rec.country}</span>
                )}
              </div>
              {rec.risk_level && (
