...
export function EntityOverviewSummary({
  ...
  const pepStateLabel =
    pepStatus === 'current_pep'
      ? t('entity.overview.pepState.current')
      : pepStatus === 'former_pep_in_monitoring'
        ? t('entity.overview.pepState.monitoring')
        : pepStatus === 'ex_pep'
          ? t('entity.overview.pepState.exPep')
          : canonicalPepCount > 0
            ? t('entity.overview.pepState.historical')
            : t('entity.overview.pepState.notRegistered');
  const hasAdverseMedia = (entity.adverse_media?.length || 0) > 0;
  const riskFactorLabels: Record<string, string> = {
    sanctions: t('entity.overview.riskFactor.sanctions'),
    pep: t('entity.overview.riskFactor.pep'),
    adverse_media: t('entity.overview.riskFactor.adverse_media'),
    geographic: t('entity.overview.riskFactor.geographic'),
    network: t('entity.overview.riskFactor.network'),
    transactional: t('entity.overview.riskFactor.transactional'),
  };
    ...
    <>
      {!hasSanctions && !isPep && !hasAdverseMedia && criticalFactors.length === 0 ? (
        <motion.div {...fadeUp} className="glass rounded-xl p-5 border-l-4 border-green-500/60">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-foreground font-medium">{t('entity.overview.noActiveAlerts')}</p>
              <p className="text-sm text-muted-foreground">
                {t('entity.overview.noActiveAlertsDescription', { count: entity.data_sources.length })}
              </p>
            </div>
          </div>
        </motion.div>
        ...
        >
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <p className="text-foreground font-medium mb-2">{t('entity.overview.alertSummary')}</p>
              <div className="flex flex-wrap gap-3">
                {hasSanctions && (
                  <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-red-500/10 border border-red-500/20">
                    <Shield className="w-4 h-4 text-red-600 dark:text-red-400" />
                    <span className="text-sm text-red-600 dark:text-red-300 font-medium">
                      {t('entity.overview.activeSanctions', { count: activeSanctions.length })}
                    </span>
                  </div>
                )}
                {isPep && (
                  <div className={cn(
                    ...
                    <span className={cn('text-sm font-medium', pepStatus === 'former_pep_in_monitoring' ? 'text-amber-200' : 'text-purple-600 dark:text-purple-300')}>
                      {pepStatus === 'former_pep_in_monitoring' ? t('entity.overview.exPepMonitoring') : t('entity.overview.pepPerson')}
                    </span>
                  </div>
                )}
                {hasAdverseMedia && (
                  <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-amber-500/10 border border-amber-500/20">
                    ...
                    <span className="text-sm text-amber-700 dark:text-amber-300 font-medium">{t('entity.overview.adverseMedia')}</span>
                  </div>
                )}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1.5fr_1fr] gap-6 min-w-0">
        <motion.div {...fadeUp} transition={{ delay: 0.03 }} className="glass rounded-xl p-6">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div className="min-w-0">
              <p className="text-xs text-muted-foreground uppercase tracking-wide mb-2">{t('entity.overview.executiveReading')}</p>
            ...
            >
              {t('entity.overview.riskLabel', { level: riskLevelLabels[entity.risk_level] })}
            </Badge>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mt-5">
            <div className="rounded-lg bg-foreground/[0.03] border border-foreground/5 p-3">
              <p className="text-[11px] text-muted-foreground uppercase tracking-wide">{t('entity.overview.stat.activeSanctions')}</p>
              <p className="text-2xl font-bold text-foreground mt-1">{activeSanctions.length}</p>
            </div>
            <div className="rounded-lg bg-foreground/[0.03] border border-foreground/5 p-3">
              <p className="text-[11px] text-muted-foreground uppercase tracking-wide">{t('entity.overview.stat.pepStatus')}</p>
              <p className="text-sm font-semibold text-foreground mt-2">
                {pepStateLabel}
              </p>
            </div>
            <div className="rounded-lg bg-foreground/[0.03] border border-foreground/5 p-3">
              <p className="text-[11px] text-muted-foreground uppercase tracking-wide">{t('entity.overview.stat.adverseMedia')}</p>
              <p className="text-2xl font-bold text-foreground mt-1">{entity.adverse_media?.length || 0}</p>
            </div>
            <div className="rounded-lg bg-foreground/[0.03] border border-foreground/5 p-3">
              <p className="text-[11px] text-muted-foreground uppercase tracking-wide">{t('entity.overview.stat.relationships')}</p>
        ...
        <motion.div {...fadeUp} transition={{ delay: 0.05 }} className="glass rounded-xl p-6">
          <p className="text-xs text-muted-foreground uppercase tracking-wide mb-3">{t('entity.overview.coverage')}</p>
          <div className="space-y-3">
            <div className="flex items-start justify-between gap-3">
              <span className="text-sm text-muted-foreground">{t('entity.overview.activeSources')}</span>
            ...
            <div className="flex items-start justify-between gap-3">
              <span className="text-sm text-muted-foreground">{t('entity.overview.datasets')}</span>
              <span className="text-sm font-semibold text-foreground text-right break-words">
                {profile?.cross_references?.datasets?.length || entity.source_records?.length || 0}
              </span>
            </div>
            <div className="flex items-start justify-between gap-3">
              <span className="text-sm text-muted-foreground">{t('entity.overview.firstSeen')}</span>
              <span className="text-sm font-semibold text-foreground text-right break-words">
                {profile?.first_seen_at || entity.first_seen ? formatDate(profile?.first_seen_at || entity.first_seen) : '—'}
              </span>
            </div>
            <div className="flex items-start justify-between gap-3">
              <span className="text-sm text-muted-foreground">{t('entity.overview.lastUpdated')}</span>
              <span className="text-sm font-semibold text-foreground text-right break-words">
          ...
          {relationshipSignals.length > 0 && (
            <div className="mt-5 pt-4 border-t border-foreground/5">
              <p className="text-xs text-muted-foreground uppercase tracking-wide mb-2">{t('entity.overview.relationalSignals')}</p>
