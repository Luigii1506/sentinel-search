...
const ROLE_COLOR: Record<UserRoleName, string> = {
  ...
  readonly: 'bg-navy-600 text-navy-100 border-navy-500',
};
  ...
  const columns: DataTableColumn<AppUser>[] = [
    {
      ...
      cell: (u) => (
        <div className="space-y-0.5">
          ...
          {u.email && <div className="text-xs text-navy-200">{u.email}</div>}
        </div>
      ),
    },
    {
            ...
            {ROLE_OPTIONS.map((opt) => (
              <SelectItem key={opt.value} value={opt.value}>
                <div className="flex flex-col">
                  ...
                  <span className="text-[11px] text-navy-200">{opt.description}</span>
                </div>
              </SelectItem>
            ))}
          </SelectContent>
    ...
    },
    {
      ...
      cell: (u) => (
        u.is_active ? (
          ...
          <span className="inline-flex items-center gap-1 text-xs text-navy-200">
    ...
    },
    {
      ...
      header: 'Creado',
      hideOnMobile: true,
      cell: (u) => <span className="text-xs text-navy-100">{formatDate(u.created_at)}</span>,
    },
    {
      id: 'last_login',
      header: 'Último login',
      cell: (u) => <span className="text-xs text-navy-100">{formatDate(u.last_login, 'Nunca')}</span>,
    },
    {
      id: 'actions',
      header: '',
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={(o) => !o && setDeleteTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              <strong className="text-white">{deleteTarget?.username}</strong> perderá
