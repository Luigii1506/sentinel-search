...
import { useMergeReview, useMergeDetail } from '@/hooks/useMergeReview';
...
export function MergeReviewPage() {
  ...
  const { data, isLoading, error, refetch } = useMergeReview({
    limit: PAGE_SIZE,
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            <Card className="bg-white/5 border-white/5">
              <CardContent className="p-4">
                <p className="text-xs text-gray-500 mb-1">Total Merges</p>
                <p className="text-2xl font-bold text-white">
                  {(stats.total_merged_entities || 0).toLocaleString()}
                </p>
              </CardContent>
            </Card>
            <Card className="bg-blue-500/5 border-blue-500/10">
              <CardContent className="p-4">
                <p className="text-xs text-blue-400 mb-1">Cross-Source</p>
                <p className="text-2xl font-bold text-blue-400">
                  {(stats.cross_source_merges || 0).toLocaleString()}
                </p>
              </CardContent>
            </Card>
            <Card className="bg-purple-500/5 border-purple-500/10">
              <CardContent className="p-4">
                <p className="text-xs text-purple-400 mb-1">Gold Dedup</p>
                <p className="text-2xl font-bold text-purple-400">
                  {(stats.gold_dedup_merges || 0).toLocaleString()}
                </p>
              </CardContent>
            </Card>
            <Card className="bg-green-500/5 border-green-500/10">
              <CardContent className="p-4">
                <p className="text-xs text-green-400 mb-1">ER v4</p>
                <p className="text-2xl font-bold text-green-400">
                  {(stats.er_v4_merges || 0).toLocaleString()}
                </p>
              </CardContent>
            </Card>
          </div>
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <div className="relative">
                ...
                <Input
                  ...
                  className="pl-9 w-[200px] bg-white/5 border-white/10 text-gray-300 placeholder:text-gray-600"
                />
              </div>
            </div>
          </CardContent>
          ...
          <>
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-white/5">
                  {data.entities.map((entity: MergedEntitySummary, index: number) => {
                    ...
                    return (
                      <AnimatePresence key={entity.entity_id}>
                        <motion.tr
                          initial={{ opacity: 0 }}
                          animate={{ opacity: 1 }}
                        ...
                        {/* Expanded detail */}
                        {isExpanded && (
                          <tr key={`${entity.entity_id}-detail`}>
            ...
            <div className="flex items-center justify-between mt-4 px-2">
              <p className="text-sm text-gray-500">
                ...
                {data.total.toLocaleString()} entidades
              </p>
