...
export function ActivityLogPage() {
  ...
  const columns: DataTableColumn<AuditLogEntry>[] = [
    {
      id: 'when',
      header: t('insights.activity.columns.when'),
      cell: (row) => (
    ...
    {
      id: 'user',
      header: t('insights.activity.columns.user'),
      primary: true,
      cell: (row) => (
        <span className="text-xs">
          <span className="text-foreground font-medium">
            {row.username ?? <span className="text-muted-foreground">{t('insights.activity.anonymous')}</span>}
          </span>
        </span>
      ),
    },
    {
      id: 'auth',
      header: t('insights.activity.columns.auth'),
      hideOnMobile: true,
      cell: (row) => <span className="text-xs text-muted-foreground">{row.auth_method ?? '—'}</span>,
    },
    {
      id: 'method',
      header: t('insights.activity.columns.method'),
      cell: (row) => (
        <Badge
          variant="outline"
    ...
    {
      id: 'endpoint',
      header: t('insights.activity.columns.endpoint'),
      cell: (row) => (
    ...
    {
      id: 'status',
      header: t('insights.activity.columns.status'),
      align: 'right',
      cell: (row) => {
    ...
    {
      id: 'latency',
      header: t('insights.activity.columns.latency'),
      align: 'right',
      cell: (row) => (
    ...
    {
      id: 'ip',
      header: t('insights.activity.columns.ip'),
      hideOnMobile: true,
      cell: (row) => (
  ...
  return (
    <AppPage>
        <PageHeader
          title={t('insights.activity.title')}
          description={t('insights.activity.description')}
          icon={
          ...
          actions={
            <Button
              ...
              disabled={isFetching}
              className="gap-2"
              aria-label={t('insights.activity.refreshAria')}
            >
        ...
        {stats && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            <MetricCard label={t('insights.activity.metrics.events')} value={stats.total.toLocaleString()} delay={0} />
            <MetricCard label={t('insights.activity.metrics.uniqueUsers')} value={stats.uniqueUsers} delay={0.05} />
            <MetricCard
              label={t('insights.activity.metrics.errors')}
              value={stats.errors}
              accent={stats.errors > 0 ? 'amber' : undefined}
              delay={0.1}
            />
            <MetricCard label={t('insights.activity.metrics.p95Latency')} value={stats.p95Latency} unit="ms" delay={0.15} />
          </div>
        )}
        ...
        <Card>
          <CardContent className="p-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            <div className="relative">
              ...
              <Input
                placeholder={t('insights.activity.filters.usernamePlaceholder')}
                value={usernameFilter}
                onChange={(e) => setUsernameFilter(e.target.value)}
                className="pl-8"
                aria-label={t('insights.activity.filters.usernameAria')}
              />
            </div>
            <div className="relative">
              <Globe className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-muted-foreground" aria-hidden="true" />
              <Input
                placeholder={t('insights.activity.filters.endpointPlaceholder')}
                value={endpointFilter}
                onChange={(e) => setEndpointFilter(e.target.value)}
                className="pl-8"
                aria-label={t('insights.activity.filters.endpointAria')}
              />
            </div>
            <Select value={methodFilter} onValueChange={setMethodFilter}>
              <SelectTrigger aria-label={t('insights.activity.filters.methodAria')}>
                <SelectValue placeholder={t('insights.activity.filters.methodPlaceholder')} />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">{t('insights.activity.filters.allMethods')}</SelectItem>
            ...
            <Select value={String(limit)} onValueChange={(v) => setLimit(Number(v))}>
              <SelectTrigger aria-label={t('insights.activity.filters.limitAria')}>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                {LIMIT_OPTIONS.map((l) => (
                  <SelectItem key={l} value={String(l)}>
                    {t('insights.activity.filters.lastN', { count: l })}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
          ...
          <CardContent className="p-0">
            <DataTable<AuditLogEntry>
              ...
              empty={
                <EmptyState
                  icon={Search}
                  title={t('insights.activity.empty.title')}
                  description={t('insights.activity.empty.description')}
                />
              }
            />
          </CardContent>
